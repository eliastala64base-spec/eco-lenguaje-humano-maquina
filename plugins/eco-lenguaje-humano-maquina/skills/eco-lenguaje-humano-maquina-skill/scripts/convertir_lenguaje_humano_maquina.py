...
def build_document(unit_dir: Path, scope: str, base: str, records: list[dict[str, Any]], selected: dict[str, dict[str, Any]], warnings: list[str]) -> None:
    ...
    md = ["---", f"nombre_base_exacto: {json.dumps(base, ensure_ascii=False)}", f"revision_canonica: {json.dumps(revision)}", "estado: DERIVADO_GENERADO", "---", "", f"# {base}", "", "> Derivado navegable. Los originales son la autoridad.", "", "## Fuente principal", "", f"- Revisión: `{revision}`", f"- Archivo: `{primary['nombre_archivo'] if primary else 'NINGUNA_FUENTE_LEGIBLE'}`", "", "## Contenido", "", resource["contenido"]["texto"] or "*Sin extracción textual verificable; revise la fuente original y los controles.*", "", "## Trazabilidad", "", "- [Recurso canónico](02_RECURSO_CANONICO.json)", "- [Relaciones Graphiti](03_RELACIONES_GRAPHITI.jsonld)", "- [Fuentes asociadas](90_CONTROL_DOCUMENTO/fuentes_asociadas.json)"]
    markdown = "\n".join(md) + "\n"
